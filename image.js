...
import { canUseAgent, inTrial } from "../access.js";
...
router.post("/", requireAuth, async (req, res) => {
  ...
  // Image generation is Iris (the "image" agent). Enforce plan/trial access.
  const freshUser = await db.findById(req.user.id);
  if (!canUseAgent(freshUser, "image")) {
    const reason = (freshUser?.plan === "trial" && !inTrial(freshUser))
      ? "Your free trial has ended. Upgrade to keep generating images."
      : "Image generation is on the Artist plan and up. Upgrade to unlock it.";
