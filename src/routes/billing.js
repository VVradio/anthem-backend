...
router.post("/checkout", requireAuth, async (req, res) => {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.CLIENT_ORIGIN}/dashboard?paid=1`,
...
export async function stripeWebhook(req, res) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (userId && plan) db.setPlan(userId, plan);
    ...
    if (user?.referredBy && plan) {
      db.recordReferralConversion(user.email, plan, PLAN_CENTS[plan] || 0);
    }
  }
...
router.post("/connect/onboard", requireAuth, async (req, res) => {
  try {
    let acctId = db.findById(req.user.id)?.stripeConnectId;
    if (!acctId) {
      const account = await stripe.accounts.create({ type: "express", email: req.user.email });
      acctId = account.id;
      db.setConnectAccount(req.user.id, acctId);
    }
...
router.post("/payout", requireAuth, async (req, res) => {
  ...
  const owedCents = db.commissionOwed(user.referralCode);
